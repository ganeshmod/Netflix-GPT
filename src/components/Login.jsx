...
import { obj } from '../utils/Validation';
...
function Login() {
    ...
    const handleSubmit = (e) => {
        ...
        const nameValue = nameR.current?.value || "";;
        console.log('sign in value ', signIn);
        const msg = signIn ? obj.ValidateData(emailValue, passwordValue) : obj.ValidateDataWithName(nameValue, emailValue, passwordValue);
        ...
        <div>
            ...
            <div className='relative  '>
                ...
                <form onSubmit={handleSubmit} className='absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex w-3/12 mx-4 flex-col gap-4 bg-black/80 p-6 rounded-lg shadow-2xl  bg-opacity-50'>
                    ...
                    {
                        !signIn && (
                            <input ref={nameR} className='w-full rounded-md border border-gray-600 bg-gray-900 px-4 py-2 text-white placeholder-gray-400 outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500' type="text" placeholder='Enter your Name' />
                        )
                    }
                    <input
                        ref={email}
                    ...
                    <p
                        ...
                        className='text-white my-4 px-2'>{signIn ? 'Are you new? Sign up' : 'Already have an account? Sign in'} </p>
                </form>
                <ToastContainer />
            </div>
